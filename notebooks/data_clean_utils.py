...
def data_cleaning(data: pd.DataFrame):
    ...
    return (
        data
        .drop(columns="id")
        .drop(index=minor_index)      # drop minors
        .drop(index=six_star_index)   # drop six-star ratings
        .replace("NaN ", np.nan)
        .assign(
            ...
            order_time_of_day=lambda x: x['order_time_hour'].pipe(time_of_day),  # <-- requires function
